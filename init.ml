...
(*extra test case*)
let primes_prog_unused_and_divide_by_zero = "
     read n
     cp := 2
     while n > 0
         found := 0
...
let rec ast_ize_P (p:parse_tree) : ast_sl =
  ...
  (*[]*)
  (*names based on extended calculator grammar *)
  match p with
...
and ast_ize_C (c:parse_tree) : ast_c =
  ...
  (*
     your code here ...
  *)
  | PT_nt("C", [lhs;PT_nt("rn",[PT_term rn]);rhs])->(rn, ast_ize_expr lhs, ast_ize_expr rhs)
...
(*Prologue that will be utilized and added in every generated C file*)
let prologue = "
  #include <stdio.h>
  #include <stdlib.h>

  double getreal(int ln) {
    double a;
    int i = scanf(\"%lf\",&a);
    // returns a real number from standard input or
    if(i == 0){
      printf(\"Not valid real number input at line %d \\n\", ln);
      exit(1);
    }else if(i == EOF){
  ...
  double divide(double a, double b, int ln){
      if(b==0.0){
        printf(\"Divided by zero exception at line: %d\\n\",ln);
      exit(1);
      }else return a/b;
    }
...
(*helper function to check if given variables not used after declaration*)
(*Extra credit #3*)
let rec check_unused (declared:vars) (used:vars) (unused:string)=
 match declared with
 | hd::tl -> if (mem hd used) then check_unused tl used unused else let tmp = check_unused tl used unused in
    (hd^" "^tmp)
 | [] -> unused
(*helper function to check if given variables not are not initilized before used*)
 let rec check_uninit (declared:vars) (used:vars) (unused:string)=
 match used with
 | hd::tl -> if (mem hd declared) then check_uninit declared tl unused else let tmp = check_uninit declared tl unused in
    (hd^" "^tmp)
 | [] -> unused
(*followed given translate format and add error or warning message*)
let rec translate (ast:ast_sl)
  :  string   * string
  (* warnings   output_program *)  =
   let declared, used, tmp, ln = translate_sl ast [] [] 0 in
    let un_ini = (check_uninit declared used "") in
    let un_used = (check_unused declared used "") in
    match un_used with
    | "" -> (match un_ini with 
      | "" -> ("", prologue^ tmp ^ "}\n")
      | _ -> ("The following varibales are not initilized: " ^ un_ini^"\n", prologue^ tmp ^ "}\n"))
    | _ -> (match un_ini with 
      | "" -> ("The following varibales are not been used: " ^ un_used ^"\n", prologue^ tmp ^ "}\n")
      | _ -> ("The following varibales are not initilized: " ^ un_ini^"\n" ^ "The following varibales are not been used: " ^ un_used ^"\n", prologue^ tmp ^ "}\n"))

(*  commented out so this code will compile*)
(*  We followed the tail-recursion methods and I believe based on the 6 test cases we used, our translation from
AST to output C program worked successfully. All required error or warning checks are done*)
  and translate_sl (ast:ast_sl) (declared:vars) (used:vars) (ln:int): vars * vars * string * int =
  match ast with
  | hd::tl -> let declared_s, used_s, tmp, ln1 = translate_s hd declared used ln in
    let declared_sl, used_sl, tmp2, ln2 = translate_sl tl declared_s used_s ln1 in
    (declared_sl, used_sl, tmp^tmp2, ln2)
  | [] -> (declared,used, "",ln)
  (* | _ -> raise (Failure "malformed AST in translate_sl") *)
(* matched different AST_S attribute*)
and translate_s (ast:ast_s) (declared:vars) (used:vars) (ln:int): vars * vars * string * int=
  match ast with
  | AST_error -> raise (Failure "malformed AST in translate_s")
  | AST_assign (_,_)-> translate_assign ast declared used ln
  | AST_read _-> translate_read ast declared used ln
  | AST_write _-> translate_write ast declared used ln
  | AST_if (_,_)-> translate_if ast declared used ln
  | AST_while (_,_)-> translate_while ast declared used ln
  | _ -> raise (Failure "malformed AST in translate_s")

(*Following translate funncitons followed the given AST tokens or attributes and rules*)
and translate_assign (ast:ast_s) (declared:vars) (used:vars) (ln:int): vars * vars * string * int =
  match ast with 
  | AST_assign (id,expr) -> let declared_expr, used_expr, tmp, ln1 = translate_expr expr declared used ln in
    let declared2,id_str = if (mem id declared_expr) then (declared_expr,(id ^ " = "))
    else (id :: declared_expr,("double " ^id ^ " = ")) in
    (declared2, used_expr,id_str^tmp^";\n",ln1+1)
  | _ -> raise (Failure "malformed AST in translate_assign")

and translate_read (ast:ast_s) (declared:vars) (used:vars) (ln:int): vars * vars * string * int =
  match ast with
  | AST_read id -> let declared2,id_str = if (mem id declared) then (declared,(id ^ " = "))
    else ((id::declared),("double " ^id ^ " = ")) in
    (declared2, used, id_str^ "getreal("^string_of_int (ln+1)^") ;\n",ln+1)
  | _ -> raise (Failure "malformed AST in translate_read")

and translate_write (ast:ast_s) (declared:vars) (used:vars) (ln:int): vars * vars * string * int =
  match ast with
   | AST_write expr -> 
    let declared_expr, used_expr, tmp ,ln1 = translate_expr expr declared used ln in
    (declared_expr, used_expr, "putreal("^tmp^");\n", ln1+1)
  | _ -> raise (Failure "malformed AST in translate_write")

and translate_if (ast:ast_s) (declared:vars) (used:vars) (ln:int): vars * vars * string * int =
  match ast with
  | AST_if (c,sl) ->  let declared_c, used_c, tmp ,ln1 = translate_cond c declared used ln in
    let declared_sl, used_sl, tmp2, ln2 = translate_sl sl declared_c used_c (ln1+1) in
    (declared_sl,used_sl,"if (" ^ tmp^" ) { \n" ^ tmp2 ^"} \n",ln2+1)
  | _ -> raise (Failure "malformed AST in translate_if")

and translate_while (ast:ast_s) (declared:vars) (used:vars) (ln:int): vars * vars * string * int =
  match ast with 
  | AST_while (c,sl) -> let declared_c, used_c, tmp, ln1 = translate_cond c declared used ln in
    let declared_sl, used_sl, tmp2, ln2 = translate_sl sl declared_c used_c (ln1+1) in
    (declared_sl,used_sl,"while (" ^ tmp^" ) { \n" ^ tmp2 ^"} \n", ln2+1)
  | _ -> raise (Failure "malformed AST in translate_while")


and translate_expr (ast:ast_e) (declared:vars) (used:vars) (ln:int): vars * vars * string * int =
  match ast with 
  | AST_binop ("/", e1, e2) -> let decalred1,used1,tmp1, ln1 = translate_expr e1 declared used ln in
    let decalred2,used2,tmp2,ln2 = translate_expr e2 decalred1 used1 ln1 in
    (decalred2, used2, "divide(" ^ tmp1 ^ ", " ^ tmp2 ^ "," ^ string_of_int (ln+1) ^") ", ln2)
  |AST_binop (op, e1, e2) -> let decalred1,used1,tmp1, ln1 = translate_expr e1 declared used ln in
    let decalred2,used2,tmp2,ln2 = translate_expr e2 decalred1 used1 ln1 in
    (decalred2, used2, tmp1 ^ op ^ tmp2, ln2)
  | AST_id id -> let used_e = if (mem id used) then used else id::used in
      (declared,used_e,id, ln)
  | AST_num num -> (declared,used,num,ln)
  | _ -> raise (Failure "malformed AST in translate_expr")

and translate_cond (ast:ast_c) (declared:vars) (used:vars) (ln:int): vars * vars * string * int =
  match ast with 
  | ("<>",e1,e2) -> let decalred1,used1,tmp1, ln1 = translate_expr e1 declared used ln in
    let decalred2,used2,tmp2, ln2 = translate_expr e2 decalred1 used1 ln1 in
    (decalred2, used2, tmp1 ^" != "^ tmp2 , ln2)
  | ("=",e1,e2) -> let decalred1,used1,tmp1, ln1 = translate_expr e1 declared used ln in
    let decalred2,used2,tmp2, ln2 = translate_expr e2 decalred1 used1 ln1 in
    (decalred2, used2, tmp1 ^" == "^ tmp2 ,ln2)
  | (op,e1,e2) -> let decalred1,used1,tmp1, ln1 = translate_expr e1 declared used ln in
    let decalred2,used2,tmp2, ln2 = translate_expr e2 decalred1 used1 ln1 in
    (decalred2, used2, tmp1 ^op^ tmp2 ,ln2)
  | _ -> raise (Failure "malformed AST in translate_cond");; 
...
(*Function adopted and adapted from stackoverflow*)
...
(*concatenation to get target file name*)
let filename = Sys.argv.(1) ^ ".calc";;
...
let compile (code:string) (program_name:string) =
  try
    ...
    let ofile = open_out ofile_name in
      ...
      close_out ofile;
    "Starting translate "^ program_name^ ":\n" ^errs ^ "Code written to " ^ ofile_name ^ "\n\n"
    ...
    fold_left (^) ""
      (map (fun (code, program_name) -> compile code program_name)
          (*[(sum_ave_prog, "sum_ave");
           ...
           ("a := 10", "unused_var");
           ("write 3/0", "zero_div");
           (primes_prog_unused_and_divide_by_zero, "test_extra")]*)
           [(read_whole_file filename, Sys.argv.(1))]) in
  print_string msgs;;
